...
class ComaLinkRepository:
    def __init__(self, current_user_id: Optional[str] = None):
        ...
        if not self.events and not self.class_schedule:
    ...
    def _init_default_data(self):
        ...
        self.class_schedule_by_user["user_me"] = {
            (day, slot): ClassInfo(day, slot, title, room)
            for day, slot, title, room in default_classes
        }
        self.event_participants = {e.id: {e.owner_user_id} for e in self.events}
        self.event_pending = {e.id: set() for e in self.events}
        if not self.users:
            self.users = {}
        self.users.setdefault(
            "user_me",
            {
                "password_hash": self._hash_password("user_me", "password"),
                "display_name": "ユーザーME",
            },
        )
...
class TimetableScreen(Screen):
    ...
    def build_grid(self):
        ...
        for slot_id, slot_label in SLOTS:
            ...
            for day_id, _ in DAYS:
                ...
                owner_pending = any(
                    e.owner_user_id == self.repo.current_user_id
                    and self.repo.get_event_pending(e.id)
                    for e in events
                )
                ...
                for idx, e in enumerate(events):
                    if idx < max_icons:
                        icon_chars.append(get_category_by_id(e.category_id).icon)
                extra = len(events) - len(icon_chars)
                icon_row = " ".join(icon_chars)

                lines = []
                if icon_row:
                    if extra > 0:
                        lines.append(f"[font=EMOJI]{icon_row}[/font]+{extra}")
                    else:
                        lines.append(f"[font=EMOJI]{icon_row}[/font]")
                if count > 0:
                    lines.append(f"{count}件の募集")
                lines.append(f"空き{free_users}人")
                if class_info:
                    title_text = class_info.title or "授業"
                    lines.append(f"[b]{title_text}[/b]")
                    if class_info.classroom:
                        lines.append(f"@{class_info.classroom}")
                if owner_pending:
                    lines.append("[color=ff9900]承認待ちあり[/color]")
                ...
                btn = TimeSlotButton(
                    ...
                    slot_id=slot_id,
                    text=text,
                    background_color=(0.95, 0.97, 1.0, 1) if count else (1, 1, 1, 1),
                )
    ...
    def show_slot_popup(self):
        ...
        title_lbl = JpLabel(
            text=f"{day_label} {slot_label} / 空き推定: {free_users}人",
            size_hint_y=None,
            height=dp(32),
        )
